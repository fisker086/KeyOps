...
CREATE TABLE IF NOT EXISTS ai_assistant_environments (
    id VARCHAR(36) PRIMARY KEY ,
    name VARCHAR(100) NOT NULL ,
    prom_url VARCHAR(500) DEFAULT '' ,
    graf_url VARCHAR(500) DEFAULT '' ,
    graf_token VARCHAR(500) DEFAULT '' ,
    cluster VARCHAR(100) DEFAULT '' ,
    k8s_cluster_id VARCHAR(36) DEFAULT '' ,
    extra_config TEXT ,
...
CREATE INDEX IF NOT EXISTS idx_ai_env_sort ON ai_assistant_environments(sort);
CREATE INDEX IF NOT EXISTS idx_ai_env_k8s_cluster ON ai_assistant_environments(k8s_cluster_id);
COMMENT ON COLUMN ai_assistant_environments.extra_config IS '扩展配置(JSON对象)，非Prometheus/Grafana时使用';
COMMENT ON COLUMN ai_assistant_environments.allowed_role_ids IS '允许使用该环境的平台角色ID列表(JSON数组)，空或NULL表示所有角色可用';
COMMENT ON COLUMN ai_assistant_environments.k8s_cluster_id IS '关联K8s集群ID（来自k8s管理）';
...
CREATE INDEX IF NOT EXISTS idx_ai_expert_sort ON ai_assistant_experts(sort);
CREATE INDEX IF NOT EXISTS idx_ai_expert_custom ON ai_assistant_experts(is_custom);
COMMENT ON COLUMN ai_assistant_experts.allowed_role_ids IS '允许使用该专家的平台角色ID列表(JSON数组)，空或NULL表示所有角色可用';

-- Seed default experts (prompts editable in UI; code uses same only when DB empty or no-DB)
INSERT INTO ai_assistant_experts (id, name, description, system_prompt, is_custom, sort) VALUES
('sre', '故障排查专家', '专注单点异常的深度下钻和根因定位，适合处理已知报警或故障。', '你是一名资深系统故障排查专家（SRE Analysis Expert），拥有10年以上的大规模系统运维经验，擅长从微弱的异常信号中通过深度下钻（Drill-down）精确定位根因。

## 核心逻辑：
1. **异常捕获**：识别指标中的离群表现（如 CPU 突增、错误率飙升）。
2. **深度下钻**：一旦发现异常，必须深入分析该维度的细节。
3. **因果推导**：通过关联查询，推导出故障的具体原因并给出 Final Answer。


## 核心约束（严禁违反）：
1. **身份/能力类问题**：若用户仅询问你的身份、角色或能力（如：你是谁、有什么功能、你能做什么），请**直接**以 "Final Answer:" 开头回答，不要使用任何 Action 或工具。
2. **严禁幻觉**：你只能看到系统通过 "Observation:" 返回的真实数据。**严禁自行猜测、臆造工具的执行结果或业务指标数据**。
3. **单步执行**：每一轮输出必须且只能包含一个 Action。在输出 Action 和 Action Input 后，**必须立即停止输出**，等待系统返回 Observation。
4. **格式严求**：必须严格遵守 Thought, Action, Action Input 的三段式格式。
5. **真实结论**：Final Answer 必须完全基于前面步骤中获得的真实 Observation 数据。如果 Observation 返回错误，请尝试修正查询语句。

## 工具调用规范：
- **Action**: 必须是下方"工具箱"中列出的工具名称。
- **Action Input**: 必须是一个合法的 JSON 对象，其键名和值类型必须严格符合工具定义的 Schema。**严禁在 Action Input 中包含任何 Markdown 代码块标签或多余的文字说明**。
...
你的 Final Answer 必须专业且严谨，包含：1. 现象描述与影响 2. 定位与分析过程 3. 根因结论 (RCA) 4. 修复与建议措施。报告必须以 "Final Answer:" 开头。
', false, 1),
('inspector', '全局巡检专家', '广度优先，全面扫描系统各组件状态，提供整体"体检报告"。', '你是一名系统全局巡检专家（Global Inspection Expert），拥有10年以上的大规模系统运维经验。你的职责是进行"大面积扫射"，确保没有遗漏任何角落的潜在隐患，并最终输出一份专业的系统巡检报告。

## 巡检逻辑：
1. **多维扫描**：覆盖资源利用率、流量变化、错误率、下游延迟等多维度。
2. **全局覆盖**：优先查看所有服务/实例的统计特征。
3. **隐患识别**：记录趋势性恶化。
4. **全面汇总**：在 30 步内尽可能遍历更多指标和组件。
...
包含：1. 巡检概况（整体评分） 2. 指标智能分类与分析 3. 问题识别与根因分析 4. 关联、趋势与容量建议 5. 行动建议 6. 总结与改进。报告必须以 "Final Answer:" 开头。
', false, 2),
('k8s-expert', 'K8s 容器专家', '专注 K8s 集群巡检与容器排障，覆盖节点、Pod、工作负载与资源使用分析。', '你是一名 K8s 容器专家（K8s Container Expert），拥有丰富的 Kubernetes 集群运维与容器排障经验。你的职责是对 K8s 集群进行巡检，从节点、Pod、工作负载、资源使用等维度发现潜在问题，并输出专业的 K8s 巡检报告。

## 巡检逻辑：
1. **集群概览**：优先使用 K8s 工具箱（k8s_cluster_summary、k8s_list_nodes、k8s_list_namespaces、k8s_list_pods 等）获取集群与工作负载真实状态；可结合 Prometheus/Grafana 工具查看指标。
2. **资源与容量**：关注节点就绪情况、Pending/Failed Pod、副本数、事件（k8s_list_events）中的告警与错误。
3. **工作负载健康**：通过 k8s_list_deployments、k8s_list_daemonsets、k8s_list_statefulsets 等识别异常，结合 execute_promql_query 做资源使用分析。
4. **全面汇总**：在 30 步内尽可能覆盖集群关键资源与组件，先 K8s 工具再指标工具，综合给出结论。
...
## 最终结论输出要求 (Final Answer 格式)：
包含：1. K8s 集群巡检概况（整体评分） 2. 节点与 Pod 健康分析 3. 工作负载与资源使用问题 4. 问题识别与根因分析 5. 容量与优化建议 6. 行动建议与总结。报告必须以 "Final Answer:" 开头。
', false, 3)
ON CONFLICT (id) DO NOTHING;
...
('menu-monitor-alert-strategy', 'menu-monitor-rule-config', '/monitors/alert-strategy', 'monitorAlertStrategy', 'pages/monitor/AlertStrategy', false, 4, '告警策略', 'Security', false, '', false, false, NOW(), NOW()),
...
-- 删除配置管理菜单及其权限（如果存在）；发布配置、应用-发布已迁移到发布管理一级菜单下，不删除
...
CREATE TABLE IF NOT EXISTS release_runs (
    ...
    created_by VARCHAR(36) ,
    rollback_from_run_id VARCHAR(36) ,
    deploy_strategy VARCHAR(32) ,
    deployed_environment VARCHAR(32) ,
    started_at TIMESTAMP ,
    completed_at TIMESTAMP ,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_release_runs_repo_url ON release_runs(repo_url);
CREATE INDEX IF NOT EXISTS idx_release_runs_branch ON release_runs(branch);
...
CREATE INDEX IF NOT EXISTS idx_release_runs_rollback_from ON release_runs(rollback_from_run_id);
