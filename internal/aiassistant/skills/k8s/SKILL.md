...
- `k8s_list_pods` 未给 `namespace` 时默认使用 `default`
- 其他工作负载/事件查询未给 `namespace` 时，优先先列命名空间再让用户确认目标
- 用户只说“集群异常”这类泛化问题时，先走概览流程，不直接下结论
- 需要定位对象但缺少名称时，先查列表（pods/deployments/events）再下钻

## 失败重试与降级策略

1. 工具失败先做**一次收敛重试**（缩小 namespace、补 object_kind/object_name）
2. 若仍失败，降级为“可执行排查路径”：给出下一步最小命令/工具调用建议
3. 禁止推断不存在的 K8s 资源状态；拿不到数据时明确说明“当前未检索到”
4. 如果事件为空，不代表无故障，需结合节点状态与工作负载状态交叉验证

## 标准输出结构（建议统一）

1. **结论**：当前集群是否健康、主要异常点
2. **证据**：节点/Pod/事件的关键事实（对象名、状态、时间）
3. **下一步**：建议继续调用的 K8s 或 Prometheus 工具及参数
...
- 适用角色：`inspector`、`k8s-expert`
- 评审模式：
  - `inspector`：走极速评审（2-3 分钟）
  - `k8s-expert`：在极速评审基础上，额外检查对象粒度与定位链路
- 必查三板斧：
  1. 参数兜底与默认值
  2. 失败重试与降级策略
  3. 输出结构（结论/证据/下一步）
- `k8s-expert` 额外必查：
  - 是否明确到 namespace / object_kind / object_name，避免“集群异常”式泛化结论
