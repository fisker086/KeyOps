...
type AlertRuleGroupRepository interface {
	Create(group *model.AlertRuleGroup) error
	Update(group *model.AlertRuleGroup) error
	Delete(id uint) error
	FindByID(id uint) (*model.AlertRuleGroup, error)
	List(departmentID *string) ([]model.AlertRuleGroup, error)
	ListAll(page, pageSize int) (total int64, groups []model.AlertRuleGroup, err error)
	ListBySourceID(sourceID uint) ([]model.AlertRuleGroup, error)
	FindBySourceIDAndGroupName(sourceID uint, groupName string) (*model.AlertRuleGroup, error)
	ListBySourceIDWithPagination(sourceID uint, page, pageSize int) (total int64, groups []model.AlertRuleGroup, err error)
}
...
// ListBySourceID 根据数据源ID获取规则组列表（不分页）
func (r *alertRuleGroupRepository) ListBySourceID(sourceID uint) ([]model.AlertRuleGroup, error) {
	var groups []model.AlertRuleGroup
	err := r.db.Where("source_id = ?", sourceID).Order("group_name ASC").Find(&groups).Error
	return groups, err
}

// FindBySourceIDAndGroupName 根据数据源ID和规则组名称查询
func (r *alertRuleGroupRepository) FindBySourceIDAndGroupName(sourceID uint, groupName string) (*model.AlertRuleGroup, error) {
	var group model.AlertRuleGroup
	err := r.db.Where("source_id = ? AND group_name = ?", sourceID, groupName).First(&group).Error
	return &group, err
}

// ListBySourceIDWithPagination 根据数据源ID分页获取规则组列表
func (r *alertRuleGroupRepository) ListBySourceIDWithPagination(sourceID uint, page, pageSize int) (total int64, groups []model.AlertRuleGroup, err error) {
	query := r.db.Model(&model.AlertRuleGroup{}).Where("source_id = ?", sourceID)
	if err = query.Count(&total).Error; err != nil {
		return
	}
	if total == 0 {
...
type AlertRuleSourceRepository interface {
	Create(source *model.AlertRuleSource) error
	Update(source *model.AlertRuleSource) error
	Delete(id uint) error
	FindByID(id uint) (*model.AlertRuleSource, error)
	FindByAPIKey(apiKey string) (*model.AlertRuleSource, error)
	List(page, pageSize int) (total int64, sources []model.AlertRuleSource, err error)
	ListAll(page, pageSize int) (total int64, sources []model.AlertRuleSource, err error)
	ListByDepartment(departmentID *string) ([]model.AlertRuleSource, error)
	ListByGroup(groupID *uint) ([]model.AlertRuleSource, error)
}
...
type AlertRuleRepository interface {
	...
	Delete(id uint) error
	FindByID(id uint) (*model.AlertRule, error)
	List(sourceID uint, groupID *uint, excludeGroupID *uint, group, name string, page, pageSize int) (total int64, rules []model.AlertRule, err error)
	Toggle(id uint, enabled bool) error
	ListByGroupID(groupID uint) ([]model.AlertRule, error)
}
...
func (r *alertRuleRepository) List(sourceID uint, groupID *uint, excludeGroupID *uint, group, name string, page, pageSize int) (total int64, rules []model.AlertRule, err error) {
	...
	if excludeGroupID != nil {
		// 排除指定规则组（用于「将现有规则加入本组」：只查不在本组的规则）
		query = query.Where("(group_id IS NULL OR group_id != ?)", *excludeGroupID)
	} else if groupID != nil {
...
type AlertEventRepository interface {
	Create(event *model.AlertEvent) error
	Update(event *model.AlertEvent) error
	FindByID(id uint64) (*model.AlertEvent, error)
	FindByFingerprint(fingerprint string) (*model.AlertEvent, error)
	FindSimilarEvents(fingerprint string, since time.Time) ([]model.AlertEvent, error)
	List(departmentID string, level, progress int, title, timeRange string, page, pageSize int) (total int64, events []model.AlertEvent, err error)
	Claim(id uint64, uid string) error
	CancelClaim(id uint64) error
...
type AlertLogRepository interface {
	Create(log *model.AlertLog) error
}
...
type AlertStrategyRepository interface {
	Create(strategy *model.AlertStrategy) error
...
func (r *alertStrategyRepository) Create(strategy *model.AlertStrategy) error {
	// 调试日志
	logger.Infof("[AlertStrategyRepository] Create: StrategyName=%s, Filters=%s, StrategySet=%s",
		strategy.StrategyName, string(strategy.Filters), string(strategy.StrategySet))
	err := r.db.Create(strategy).Error
	if err != nil {
		logger.Infof("[AlertStrategyRepository] Create error: %v", err)
	} else {
		logger.Infof("[AlertStrategyRepository] Create success: ID=%d", strategy.ID)
	}
	return err
}
...
type AlertAggregationRepository interface {
	Create(agg *model.AlertAggregation) error
	Update(agg *model.AlertAggregation) error
	Delete(id uint) error
	FindByID(id uint) (*model.AlertAggregation, error)
	ListEnabled() ([]model.AlertAggregation, error)
	List(page, pageSize int) (int64, []model.AlertAggregation, error)
}
...
type AlertSilenceRepository interface {
	Create(silence *model.AlertSilence) error
	Update(silence *model.AlertSilence) error
	Delete(id uint) error
...
type AlertRestrainRepository interface {
	Create(restrain *model.AlertRestrain) error
	Update(restrain *model.AlertRestrain) error
	Delete(id uint) error
	FindByID(id uint) (*model.AlertRestrain, error)
	ListAll() ([]model.AlertRestrain, error)
	List(page, pageSize int) (int64, []model.AlertRestrain, error)
}
...
type AlertTemplateRepository interface {
	...
	Delete(id uint) error
	FindByID(id uint) (*model.AlertTemplate, error)
	List() ([]model.AlertTemplate, error)
	ListWithPagination(page, pageSize int) (int64, []model.AlertTemplate, error)
}
...
type AlertChannelRepository interface {
	...
	Delete(id uint) error
	FindByID(id uint) (*model.AlertChannel, error)
	List() ([]model.AlertChannel, error)
	ListWithPagination(page, pageSize int) (int64, []model.AlertChannel, error)
}
...
type StrategyLogRepository interface {
	Create(logEntry *model.StrategyLog) error
	ListByAlertID(alertID uint64) ([]model.StrategyLog, error)
	ListByAlertIDWithPagination(alertID uint64, page, pageSize int) (int64, []model.StrategyLog, error)
	FindByID(id uint) (*model.StrategyLog, error)
}
...
func (r *strategyLogRepository) Create(logEntry *model.StrategyLog) error {
	logger.Infof("[StrategyLogRepository] Creating strategy log: alertID=%d, strategyID=%d, isNotify=%v, notifyType=%d", logEntry.AlertID, logEntry.StrategyID, logEntry.IsNotify, logEntry.NotifyType)
	err := r.db.Create(logEntry).Error
	if err != nil {
		logger.Infof("[StrategyLogRepository] Failed to create strategy log: alertID=%d, strategyID=%d, error=%v", logEntry.AlertID, logEntry.StrategyID, err)
	} else {
		logger.Infof("[StrategyLogRepository] Successfully created strategy log: alertID=%d, strategyID=%d, logID=%d", logEntry.AlertID, logEntry.StrategyID, logEntry.ID)
	}
	return err
}
...
func (r *strategyLogRepository) ListByAlertIDWithPagination(alertID uint64, page, pageSize int) (int64, []model.StrategyLog, error) {
	...
	if alertID > 0 {
		...
		logger.Infof("[StrategyLogRepository] Querying strategy logs with filter: alertID=%d, page=%d, pageSize=%d", alertID, page, pageSize)
	} else {
		logger.Infof("[StrategyLogRepository] Querying all strategy logs: page=%d, pageSize=%d", page, pageSize)
	}
	err := query.Count(&total).Offset((page - 1) * pageSize).Limit(pageSize).Order("created_at DESC").Find(&logs).Error
	if err != nil {
		logger.Infof("[StrategyLogRepository] Query error: alertID=%d, error=%v", alertID, err)
	} else {
		logger.Infof("[StrategyLogRepository] Query result: alertID=%d, total=%d, found=%d logs", alertID, total, len(logs))
	}
...
type AlertGroupRepository interface {
	Create(group *model.AlertGroup) error
	Update(group *model.AlertGroup) error
	Delete(id uint) error
	FindByID(id uint) (*model.AlertGroup, error)
	List(departmentID *string, page, pageSize int) (total int64, groups []model.AlertGroup, err error)
	ListAll(departmentID *string) ([]model.AlertGroup, error)
}
...
type ChannelTemplateRepository interface {
	Create(channelTemplate *model.ChannelTemplate) error
	Update(channelTemplate *model.ChannelTemplate) error
	Delete(id uint) error
	FindByID(id uint) (*model.ChannelTemplate, error)
	FindByTemplateIDAndChannelID(templateID, channelID uint) (*model.ChannelTemplate, error)
	ListByTemplateID(templateID uint) ([]model.ChannelTemplate, error)
	DeleteByTemplateID(templateID uint) error
	DeleteByTemplateIDAndChannelID(templateID, channelID uint) error
}
