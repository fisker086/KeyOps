...
func (h *ResourcesHandler) ExpensesBreakdown(c *gin.Context) {
	...
	costType := c.DefaultQuery("cost_type", "unblended")
	currency := billService.NormalizeDisplayCurrency(c.Query("currency"))
	...
	result, err := h.service.GetExpensesBreakdown(startDate, endDate, granularity, groupBy, vendor, serviceCode, keyword, costType, currency)
	if err != nil {
